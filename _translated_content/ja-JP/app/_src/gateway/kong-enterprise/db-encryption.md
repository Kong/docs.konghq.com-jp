...
#### ライセンス ペイロードの暗号化を構成する

デフォルトでは、 {{site.base_gateway}} は データベース内のライセンスデータを暗号化しません。 デプロイにこれが必要な場合は、以下の を kong.conf に追加することで有効にできます。
...
すべての デプロイに適していない可能性があり、注意して有効にする必要があるため、ライセンス暗号化はデフォルトで有効になっていません。

* ライセンスの暗号化を有効にしないと、 {{site.base_gateway}} がトラフィックの提供を開始する前に、データベースに保存されているライセンスを起動時に することができます。
* ライセンスの暗号化が有効な場合、キーリング のアクティベーションが完了するまで、ライセンスのアクティベーションは遅延します（クラスタ/ボールト戦略または手動リカバリのいずれか）。そのため、 {{site.base_gateway}} が開始してからエンタープライズ のすべての機能が完全にアクティベートされるまでの間に、 遅延が発生します。 この制限は、KONG_LICENSE_DATAまたはKONG_LICENSE_PATH 環境変数を介して起動時に有効なライセンスを ノードに提供することで回避できます{% endif_version %}
